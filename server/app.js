...
Meteor.methods({
	addFolder: function (userInfo) {
		...
		var dir_path = path.resolve(process.env.PWD + '/../repos/' + userInfo.id + '/' + userInfo.repo_id + '/' + dir_name);
		while (fs.existsSync(dir_path)) {
			...
			dir_path = path.resolve(process.env.PWD + '/../repos/' + userInfo.id + '/' + userInfo.repo_id + '/' + dir_name);
		}
		...
		try {
			...
			repo.file_structure.push({name: dir_name, directory: true});
			Repos.update(
				{"_id": userInfo.repo_id},
